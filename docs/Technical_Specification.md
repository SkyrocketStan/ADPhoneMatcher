...
- **Входной файл**:
  ...
  - **Проверка аномалий в telephoneNumber**:
    - Номера проверяются на длину (задаётся в `config.py` через `VALID_PHONE_NUMBER_LENGTH`, по умолчанию 6 цифр) и содержание (только цифры после нормализации).
    - Пустое поле telephoneNumber считается штатным и не логируется.
    - Аномалии (буквы, неверная длина) записываются в лог `logs/anomalies_YYYY-MM-DD_HH-MM-SS.log` с полной строкой AD.
